...
class PrioritizedReplayBuffer:
    ...
    *Note that we are indexing starting from 1*.
    ...
    def __init__(self, cfg, capacity, alpha):
        """
        ### Initialize
        """
        self.cfg = cfg
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        e = self.experience(idx, obs,  action, reward, next_obs, done)
        # self.data.append(e)
        self.data[idx] = e
    ...
    def sample(self, batch_size, beta):
        ...
        # exp = [t for t in self.data if t.idx in samples['indexes']]
        # experiences = []
        # for id in samples['indexes']:
        #     experiences.extend([t for t in exp if t.idx == id])
        experiences = [self.data[i] for i in samples['indexes']]

        # exp = []
        # for id in sorted(samples['indexes']):
        #     exp.extend([t for t in self.memory if t.idx == id])
        ...
        weights = torch.from_numpy(samples['weights']).float().to(self.device)

        tuple = (states, actions, rewards, next_states, dones, weights, samples['indexes'])
